...
class Multi2Submap2D : public Submap2D {
 public:
  Multi2Submap2D(const Eigen::Vector2f& origin, std::unique_ptr<Grid2D> grid,
           ValueConversionTables* conversion_tables);
  explicit Multi2Submap2D(const proto::Submap2D& proto,
                    ValueConversionTables* conversion_tables);
...
class Multi4Submap2D : public Submap2D {
 public:
  Multi4Submap2D(const Eigen::Vector2f& origin, std::unique_ptr<Grid2D> grid,
           ValueConversionTables* conversion_tables);
  explicit Multi4Submap2D(const proto::Submap2D& proto,
                    ValueConversionTables* conversion_tables);
...
class ActiveSubmaps2D {
 public:
  ...
  std::vector<std::shared_ptr<const Multi2Submap2D>> Multi2submaps() const;
  std::vector<std::shared_ptr<const Multi4Submap2D>> Multi4submaps() const;

 private:
  std::unique_ptr<RangeDataInserterInterface> CreateRangeDataInserter();
  std::unique_ptr<GridInterface> CreateGrid(const Eigen::Vector2f& origin);
  std::unique_ptr<GridInterface> Create2ResolutionGrid(const Eigen::Vector2f& origin);
  std::unique_ptr<GridInterface> Create4ResolutionGrid(const Eigen::Vector2f& origin);
  void FinishSubmap();
  ...
  std::vector<std::shared_ptr<Multi2Submap2D>> multi_2_submaps_;
  std::vector<std::shared_ptr<Multi4Submap2D>> multi_4_submaps_;
